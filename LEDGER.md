...
(TTAN, CHWY, plus four non-panelled names) needed the close of 2026-09-09 to resolve —
deferred one update, now scored. **n=15 for panelled calls.**
...
Edge**. The panel has made zero directional calls across fifteen tries, including on
TTAN, its tightest-ever agreement (disparity 8.7, all seven personas independently
positive) — it still sat under the ±25 Lean threshold, and the stock then moved −29.98%,
the largest magnitude break scored to date. The panelled hit rate slipped from 62%
(8/13) to 60% (9/15) this update: CHWY hit (near-exact on both band and implied move)
but TTAN missed by the widest margin in the ledger — a real beat-and-raise-adjacent
print (EPS +11.1%, revenue +2.67%) that still crashed on a ~200bps GTV deceleration,
which the panel's own red-team persona named as the live risk but scored only +8,
nowhere near enough to move the mildly-bullish (+15) consensus. **On independent events
(collapsing the ABM/UNFI double-count), the picture moved from 6/11 (55%) to 7/13
(54%)** — TTAN and CHWY are each a fresh, independent event, so this update adds one hit
and one miss to the independent-event count without changing the double-counting
problem itself. The preliminary read went 1/2 on the new calls (SIG, BRZE, SAIL hit;
TTAN, CHWY, ASO missed — 3/6 for the full day), pulling its running rate from 41%
(12/29) to 43% (15/35). At n=13 independent panelled events, the panel and the cheap
prelim read remain within a few points of each other — not enough to call a structural
edge for the expensive stage, and TTAN is a reminder that a tightly-aligned panel is not
...
One qualitative bright spot, now with a fourth (and sharpest) case: CXM (09-01), AI
(09-02), ABM (09-04 *and* 09-07), and now TTAN (09-08) are cases where a persona's
*specific stated mechanism* — not just the direction — played out exactly as described
(CXM: a soft guide undercutting a technical beat, from the red-team; AI: a
short-covering bid absorbing a weak guide, also from the red-team; ABM: the red-team's
own named "what would break my bearish call" scenario — EPS at or above $1.04, margin up
YoY, guidance floor raised — happened almost to the letter, in independently-sourced
form, in both the 09-04 and 09-07 panels; TTAN: the red-team's own named risk to its
mild-bull call — "if the SaaS de-rate is still mid-cycle, a good quarter gets sold
anyway, exactly as it did for Salesforce and Workday" — is close to exactly what
happened, on a beat that still crashed −29.98% on growth deceleration). These still land
on different sides of the Neutral hit/miss line (CXM and TTAN missed on magnitude, AI and
ABM hit), which is itself informative: getting the mechanism right is not the same as
getting the below-expected-move sizing right, and TTAN shows the failure mode plainly —
the red-team named the correct story but scored it +8 (net positive), nowhere near
enough to move a consensus that stayed mildly bullish until the stock fell almost 30%.
This is now the ledger's most common single failure pattern: a specific, later-correct
red-team mechanism carrying too little weight in the consensus score.
...
below-expected-move convention rather than the tiering's intended target. High-certainty
calls hit 43% (3/7, up slightly from 40% but still well below Med) while Med-certainty
calls hold at 75% (6/8) — and TTAN's High-certainty miss was the largest magnitude break
in the ledger, which argues the gap is not closing even where the raw percentage ticked
up. Caveat exactly as before: two of Med's eight calls are the same two events counted
twice (ABM/UNFI, both runs), so independent-event n for Med is really 6, not 8, and the
true hit rate on independent events is 4/6 (67%) — still comfortably above High's 43%
(all seven High-tier calls are independent events, no duplication), on a pattern that has
now held in the same direction across five consecutive ledger updates. "High-certainty
calls hit less often than Med" is no longer a small-sample curiosity; `scripts/synthesize.py`'s
certainty logic needs a direct look at why High correlates with worse outcomes here,
rather than more waiting for a bigger sample to resolve it on its own. TTAN is the
sharpest single data point for this yet: the tightest-ever panel agreement (disparity
8.7) produced a High-certainty call that missed by the widest margin scored to date —
tight agreement measured tight consensus on direction, not correctness.
...
| 2026-09-08 | 6 (TTAN, CHWY, SAIL, BRZE, SIG, ASO) | 2 (TTAN, CHWY) | Panelled 1/2 (50%, Neutral-hit convention); Prelim 3/6 (50%, all researched names) | n/a — both panelled calls Neutral, no signed estimate | TTAN and BRZE reported AMC 09-08; CHWY, SAIL, SIG and ASO BMO 09-09 — all resolve on the same 09-08→09-09 close-to-close window, all reported on schedule. TTAN: EPS beat +11.1%, revenue beat +2.67%, but GTV growth decelerated ~200bps to 17% YoY; closed $81.58→$57.12, **−29.98%**, the largest magnitude break scored to date (implied 13.14%, band 5.8-18.4%) — Neutral call missed badly despite the panel's tightest-ever agreement (disparity 8.7, all seven personas independently mildly bullish, certainty High); the red-team's own named risk ("if the SaaS de-rate is still mid-cycle, a good quarter gets sold anyway, exactly as it did for Salesforce and Workday") is close to exactly what happened — reversal fired, but scored only +8, nowhere near enough to move the consensus off its mild bullish lean; prelim (+25) also missed. CHWY: EPS in line, revenue beat at the top of guide, FY26 guidance raised and narrowed — a clean print — yet closed $23.27→$20.75, **−10.83%**, near-exactly the 10.65% implied move and inside the 4.7-17.4% band — Neutral call hit; red-team's "revenue already at the top of guide, no room to beat" framing matches what happened, and its own named upside-reversal case (buyback-fuelled short squeeze) did not fire; prelim (+16) wrong-signed. SAIL: EPS beat, revenue slight miss; gapped down but rallied intraday, closed near-flat −1.18%; prelim (−18) right-signed on a near-noise move. BRZE: beat on both lines and raised FY guidance, but a soft Q3 EPS guide and slowing customer adds drove −21.73%; prelim (−15) right-signed, the strongest prelim call of the run. SIG: large EPS beat, FY27 guidance raised meaningfully, amplified by a >18%-of-float short base; closed +23.96%, the day's largest move; prelim (+18) right-signed. ASO: EPS beat and a raised FY guide drove +14.40%; prelim (−12) wrong-signed. See `research/2026/09/2026-09-08/05-outcome.md` for the full detail. |
