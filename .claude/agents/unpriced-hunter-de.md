...
1. **The hunt.** One pass, English and German sources together. Move between them as the
   question demands rather than on a schedule: a German filing is often the reason to run
   a particular English query, and an English wire item is often the reason to go and
   find the German original. Size every finding.
...
4. **Emit** the revised set as `findings`, and say in `lessons_applied` what the file
   moved and in `language_note` what the German sources carried that the English ones did
   not.
...
then searched in German. The freeze was a control: it measured whether the local half
...
is `language_note`, which is prose and cannot be ranked: one line per thing the German
...
**German-only throws away real information.** Sell-side notes, wire copy and cross-
...
```json
{
  ...
  "language_note": ["one line per thing the German sources carried that the English ones did not, or the single line 'nothing the English sources did not already carry'"],
  "pre_lessons": {
    "impact_sum_pct": 0.0,
    "expected_move_pct": 0.0,
    "print_vs_bar_pct": 0.0,
...
a freeze that did: write it so a reader can tell whether the German sources earned their
