...
description: Hunts for information about a French-listed company reporting earnings imminently that the market does not appear to have priced. Runs ONE combined English-and-FRENCH pass over communiques, chiffre d'affaires trimestriel and the French press, and returns findings carrying signed expected-impact numbers in percentage points, never direction labels. Runs isolated, one instance per hunt; give it the ticker, the event window and the path to the sealed baseline.
...
1. **The hunt.** One pass, English and French sources together. Move between them as the
   question demands rather than on a schedule: a French filing is often the reason to run
   a particular English query, and an English wire item is often the reason to go and
   find the French original. Size every finding.
...
4. **Emit** the revised set as `findings`, and say in `lessons_applied` what the file
   moved and in `language_note` what the French sources carried that the English ones did
   not.
...
then searched in French. The freeze was a control: it measured whether the local half
...
is `language_note`, which is prose and cannot be ranked: one line per thing the French
...
**French-only throws away real information.** Sell-side notes, wire copy and cross-
...
```json
{
  ...
  "language_note": ["one line per thing the French sources carried that the English ones did not, or the single line 'nothing the English sources did not already carry'"],
  "pre_lessons": {
    "impact_sum_pct": 0.0,
    "expected_move_pct": 0.0,
    "print_vs_bar_pct": 0.0,
