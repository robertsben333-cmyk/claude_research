...
description: Hunts for information about a UK-listed company reporting earnings imminently that the market does not appear to have priced. Runs an English pass, freezes it, then a DOMESTIC-SOURCE pass over RNS, Investegate and the UK trade press, and returns findings carrying signed expected-impact numbers in percentage points, never direction labels. Runs isolated, one instance per hunt; give it the EPIC, the event window and the path to the sealed baseline.
...
3. **The second pass.** Re-run the hunt over DOMESTIC UK sources -- RNS, Investegate, Citywire, Proactive, Sharecast, the Investors' Chronicle, the domestic trade press. Revise: add findings, re-size existing ones,
...
that reads only English-domestic sources would miss them. Both passes are required
...
## The second pass: domestic sources, not a second language

**For the UK this is a degenerate case and you should not pretend otherwise.** The local
language is English, so there is no German-to-English gap to exploit. The variable this
stage varies for the UK is therefore **source locality, not language**: your first pass
is the international, wire and sell-side view, and your second is the domestic UK
retail-and-adviser view. The mechanics are identical to the German and French hunters so
the three stay structurally comparable, and `pre_local.variable` says `source_locality`
so nothing downstream mistakes your delta for a language result. `eu_resolve.py` reports
the UK delta separately and **refuses to pool it** with the German and French ones.

Be honest about the consequence: the UK number measures a weaker effect than the other
two, and if it comes back at zero that is not evidence about language.
