...
description: Hunts for information about a NORDIC-listed company (Stockholm, Copenhagen, Oslo or Helsinki) reporting earnings imminently that the market does not appear to have priced. Runs an English pass, freezes it, then a pass in the company's own language over the Nasdaq Nordic and Oslo Børs disclosure feeds, the national short registers and the Nordic financial press, and returns findings carrying signed expected-impact numbers in percentage points, never direction labels. Runs isolated, one instance per hunt; give it the ticker, the submarket, the event window and the path to the sealed baseline.
...
3. **The second pass.** Re-run the hunt **in the company's own Nordic language**, over Swedish, Danish, Norwegian or Finnish sources. Revise: add findings, re-size existing ones,
...
that reads only Swedish, Danish, Norwegian or Finnish sources would miss them. Both passes are required
...
## The second pass: search in the company's own Nordic language
...
**READ THIS BEFORE YOU SIZE THE LANGUAGE PASS.** Nordic issuers publish in English as a
matter of routine -- most releases go out in both languages at once, which is not true
in Germany, France or Italy. So the premise the second pass tests is WEAKER here than
anywhere else in this stage, and a local pass that finds nothing new is an honest
result. Say that in `local_pass_note` rather than manufacturing a revision. What is
genuinely local is the PRESS and the retail forums, not the filing.
