...
function tabMarkt(code) {
  ...
  if (!d.runs.length) {
    return html + `<div class="card warnbox"><h3>Nog geen run op schijf</h3>
      <p>Er staat geen enkele map onder <code>research/*/*/*/${esc(d.dir)}/</code>, dus
      ...
      dus ook niet te zien. Zodra één fire publiceert, vult dit tabblad zich vanzelf.</p></div>`;
  }

  html += `<div class="mctl">
    <span class="seg" role="group" aria-label="conviction-drempel">
  ...
  if (hasVal) html += `<span class="seg" role="group" aria-label="validatieruns">
  ...
  if (!st.n) {
    /* Tegen dezelfde verzameling als de tegels erboven. Tellen tegen alle runs
       terwijl het tabblad er twee laat zien, is precies het soort getal dat een
       lezer niet kan narekenen. */
    const pend = runs.filter(r => !r.has_resolved_file && r.n_rows).length;
    html += `<div class="card warnbox"><h3>Nog niets opgelost</h3>
      ...
      <p>Van de ${runs.length} runs hebben er ${pend} nog geen
...
TABS.forEach(([name], i) => {
  ...
  /* Roving tabindex: a tablist is ONE tab stop and the arrows move inside it.
     With nineteen tabs the alternative is nineteen presses to reach the page. */
  b.tabIndex = i === 0 ? 0 : -1;
