...
#   search volume -> the Zoekvolume tab   (cache: edge/analysis/trends-cache.json)
#   the calendar  -> the Agenda tab       (dropped by the ledger once >3 days old)
if (( FEEDERS )); then
  python3 edge/scripts/edge_search_volume.py >/dev/null 2>&1 \
    || echo "update: search volume failed, keeping the last one"
  python3 edge/scripts/edge_calendar.py --days 7 >/dev/null 2>&1 \
