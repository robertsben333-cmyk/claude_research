...
## Edge hunt — 2026-09-11 — EXECUTION
- Logged at 2026-09-11 17:25 UTC
- Execution was ON (execution.enabled: true), paper endpoint, exit_mode auction_split, flatten_before_entry false. This was not a dry run.
- Step 0b (recorded above in full): 2026-09-10's book closed. Three amc legs had already been sent into this morning's opening auction by the 'Close AMC' Routine; ORCL filled there, FEIM and RH opg orders EXPIRED UNFILLED and were rescued by plain market orders at 13:34 UTC. Realised on those three: -709.23 USD (FEIM -29.14%, ORCL -6.19%, RH -1.06%). The remaining bmo leg HOFT was sent by this session into today's closing auction (cls, ca3f9aca), unrealised -1.18% at submission; that order is pending at the time of writing.
- Step 7: 3 of 4 names met the benchmark (|impact_sum| >= 3.0 and rankable). ONE order went in. CODA short 183 sh @ 10.10, notional 1,851 USD, 20.0% of 9,273 equity, 0.25% of ADV, entry 2026-09-11 -> exit 2026-09-14. Accepted and filled immediately.
- REFUSALS, all three with reason: RFIL below the conviction floor (2.50 < 3.00); CSHR not shortable at Alpaca; HAIN not shortable at Alpaca. The two refused names are the JOINT-BOTTOM, JOINT-HIGHEST-CONVICTION names of the day at -5.50 each — so once again the traded book is not the top of the conviction range but what was left after the borrow desk. Same pattern as 2026-09-10, when AENT, RENT and REF (the two strongest convictions) were all untradeable.
- Gross 20.0% of equity against a 100% target: with a single name the 20% per-name cap binds and the account is deliberately under-deployed. That is the cap working, not a failure.
