...
## Edge hunt — 2026-09-09 — FULL-UNIVERSE EXTENSION — STARTED
- Logged at 2026-09-09 16:29 UTC
- User instruction, live and explicit: hunt ALL tickers in today's window instead of the budget-selected 8. Extending the completed run rather than restarting it — the 8 hunted names keep their existing hunts, adversary verdicts and sealed baselines untouched.
- BUDGET OVERRIDE, recorded not shed. Adding 14 hunters + 14 adversaries takes the stage to ~47 subagents against config/pipeline.yaml's edge_hunt cap of 20. budget.edge_degrade_order is deliberately NOT applied: the user asked for full coverage and full coverage is the point of the request. This is a knowing breach of the configured cap, authorised by the user in-session, not a miscount.
- Adding, in hunt_priority order: LAKE 71, SHOE 66, CULP 65, LOVE 64, GLOO 62, VNCE 61, MCFT 60, IMPP 57, TEN 55, DBI 52, YB 48, AEO 44, COO 35, M 30. One hunter each — the double-hunt on the top two by hunt_priority is already spent on NAVN (86) and WLTH (83), which remain the top two of the full 22-name universe, so the two-hunter split needs no addition.
- Baselines were sealed and committed for all 22 names before ANY agent launched in this run (commit e190ac0), so the 14 new hunters read baselines that predate every finding in the run. The sealing rule holds for the extension.
- Hunter cautions carried forward from the sweep: GLOO and IMPP have baseline_history_trustworthy=false (cadence_implausible; reaction histories contaminated by non-earnings filings) and will be told so explicitly. SHOE's baseline shows n=0 prints because CIK 895447 was renamed from Shoe Carnival on 2026-06-11 and priced_in.py discarded 105 recoverable prints; that hunter will be told. CULP and LAKE carry single-estimate consensus rows the sweep flagged as likely mis-keyed rather than bearish.
- Timing: 14:55 UTC at launch. Five of the 14 are 09-09 amc (LAKE, CULP, GLOO, AEO, COO) and must be judged before the ~20:00 UTC release; the other nine are 09-10 bmo. Concurrency cap is 8 subagents, so hunters run in two waves and adversaries follow.

## Edge hunt — 2026-09-09 — RESCORED on the corrected key
- Logged at 2026-09-09 16:34 UTC
- OPEN ISSUE 2 from this run was resolved at 16:17 UTC by a separate session (hunter-only rho=0.453 p=0.006 vs mean rho=0.407 p=0.014) and edge_score.py now sums the hunter's number. Re-ran edge_score.py on the same hunts and adversary files; edge-scores.json and edge-note.md regenerated on the corrected key. No agent was re-run and no finding changed.
- The reorder is material: AVAV +3.25->+2.50, FLWS +1.50->+2.50, LMNR +1.50->0.00 exactly, SKIL -2.50->-3.00, LSAK -3.35->-2.70, KEQU -10.50->-5.00, WLTH -15.50->-10.50. SKIL and LSAK swap rank 5/6, and FOUR names clear the conviction floor rather than five (AVAV drops below). KEQU's halving is the case that prompted the fix — hunter -4.0 against adversary -15.0 on its lead finding — and it is cited in the new code comment.
- CLOCK: this session's own elapsed-time sense was wrong by ~100 minutes. date -u read 14:52 at the first publish and 16:32 on resume; the gap is idle wall-clock, not a skewed container clock (run_log.py and date -u agree, and 14:07 matched list_triggers' last_fired_at exactly). Recording it because CLAUDE.md warns about exactly this and the correct habit is re-reading the clock rather than extrapolating from an old reading.
- Also observed on rebase: a concurrent-write conflict on _run-log.md against the stage C and stage 3 sessions, resolved by keeping both sides in logged order — the same collision stage 3 recorded at 15:56. Two stages appending at EOF from the same base cannot auto-merge.


## Stage E — adversary demoted to a weekly audit, 8 names becomes 16
- Logged at 2026-09-09 16:35 UTC
- Measured over 215 findings on six resolved days: every channel by which the adversary can reach the ranking is subtractive. size_check_pct - hunter-only 0.453 vs mean 0.407. priced_in_pct - haircut 0.325, drop at >=90 gives 0.407, >=80 gives 0.328, >=70 gives 0.221, only <=50 gives 0.305, mean priced_in per name +0.046. Monotone in how much it is allowed to remove. And since impact_sum became the key it fed nothing: 8 of 20 subagents changing nothing ranked. Kept as a Monday audit (budget.adversary_audit_weekday) because it catches factually wrong findings - the 09-09 LMNR covenant misread and the FLWS short-base claim - which nothing else does; on that day 8 names, on the other four 16. edge_score.py now returns diagnostics.residual_sum null when the findings were not all judged, plus adversary_judged as a count. Today's 09-09 edge-scores.json was re-scored in place under the corrected hunter-only sizing (KEQU -10.50 to -5.00, WLTH -15.50 to -10.50, NAVN +9.75 to +10.00): the day is unresolved so nothing depended on the old numbers, but that run's edge-note.md quotes the pre-correction figures and was left as written.
## Edge hunt — 2026-09-09 — adversary pass SKIPPED for the 14 extension names
- Logged at 2026-09-09 16:46 UTC
- User instruction, live and explicit: 'we dont run the adversaries'. Recorded as a deliberate departure from budget.edge_never_degrade ('adversary_judges_every_finding_both_sides'), not an omission or a budget shed. The original 8 names keep their existing adversary verdicts; the 14 extension names get none.
- Consequence for the RANKING: none. Since the 16:17 UTC fix the key is impact_sum = the hunter's own signed sizes summed, and priced_in_pct no longer enters it. All 22 names therefore remain comparable on the ranking key regardless of adversary coverage.
- Consequence for QUALITY ASSURANCE: real, and asymmetric between the two groups. The 8 adversary-judged names had two findings factually broken — LMNR-h1#2 (covenant deferral misread by a full year, priced_in 92) and FLWS-h1#1 (a 'short base has not moved' claim contradicted by its own cited source, and four sealed-baseline fields re-reported as discoveries, priced_in 82). Nothing equivalent will be caught on the 14. Every unjudged finding also defaults to priced_in 65 in diagnostics.residual_sum, so residual_sum is not comparable across the two groups and must not be read as if it were.
- edge_brief.py --check will now report the 14 names' findings as unjudged. That is expected and correct, not a join failure — the earlier 37/37 clean join covered the original 8 only.


## Stage E — adversary and double hunt both removed, 19 names per day
- Logged at 2026-09-09 16:48 UTC
- Decision reversed on the Monday audit: no adversary pass at all. Both its numbers were measured subtractive over 215 findings (size_check hunter-only 0.453 vs mean 0.407; priced_in monotone worse the more it removes: haircut 0.325, drop>=90 0.407, >=80 0.328, >=70 0.221, only<=50 0.305; mean priced_in per name +0.046) and neither reached the key. The double hunt on the top two goes with it: over six runs twelve paired names, and the gap predicted neither the error (+0.203) nor sign-correctness (+0.028) at n=12. Stage is now 1 sweep + 19 hunters = 20, one hunter per name, 19 names against 8. Two costs recorded as standing caveats for every note: nothing checks a finding for being factually wrong, and nothing measures reproducibility - while the double hunt ran, twelve pairs had a median gap of 2.40 points and FOUR OF TWELVE had opposite signs on a key whose typical size is about 5. priced-in-adversary.md, edge_brief.py and edge_adversary_brief.py stay in the tree unused so either pass can be re-run deliberately.

## Edge hunt — 2026-09-09 — FULL UNIVERSE COMPLETE (22 of 22)
- Logged at 2026-09-09 16:59 UTC
- All 22 names in the window hunted. 24 hunts (NAVN and WLTH double-hunted from the earlier pass), 88 findings, 22 of 22 rankable, 11 clear the 3.0 conviction floor. First run of this stage to cover its whole universe rather than a budget-selected subset.
- Ranking on impact_sum: NAVN +10.00, LOVE +5.50, VNCE +4.50, DBI +3.00, AVAV +2.50, FLWS +2.50, COO +1.40, CULP +1.40, TEN +1.30, LMNR 0.00, AEO -1.00, M -1.10, IMPP -2.00, LAKE -2.70, LSAK -2.70, SKIL -3.00, SHOE -3.50, GLOO -4.00, MCFT -4.00, YB -4.50, KEQU -5.00, WLTH -10.50.
- HEADLINE DEFECT, and the most consequential thing this run produced: impact_sum is a SUM over findings, so the two double-hunted names carry roughly twice the magnitude of the rest by construction. NAVN (2 hunters, 6 findings) and WLTH (2 hunters, 9 findings) are ranks 1 and 22 and are the ONLY two names outside +/-5.5. Mean conviction 10.25 for the double-hunted pair against 2.78 for the twenty single-hunted names. The two extremes of today's table are substantially a function of how many agents were pointed at the name.
- AND IT LIKELY CONTAMINATES THE HEADLINE HISTORICAL RESULT. Every one of the six resolved runs double-hunted its top names by hunt_priority — SPWH-a/-b (09-01), MEI-1/-2 (09-02), SWBI-a/-b (09-03), UNFI and CAN (09-04), UNFI-a/-b (09-07), YQ-a/-b (09-08). So in every day of the pooled sample the highest-priority names also carry mechanically the largest conviction, and the rho=+0.514 'conviction predicts sign-correctness' result is confounded with hunter count and with hunt_priority selection. Cheap test: re-run edge_decompose.py normalising impact_sum by hunter count, or restricted to single-hunted names. Not run here — it is a re-analysis of resolved runs, outside this session's scope.
- NO ADVERSARY PASS on the 14 extension names, per user instruction and consistent with the same-day redesign (5e44be2 made it a weekly audit; a109692 removed it and the double hunt outright; config now double_hunt_top_n 0, adversary_agents_per_ticker 0, hunted_names 19). The 8 original names keep verdicts made earlier in this run. Two factual breaks in 37 checked findings (~5%) implies roughly two to three undetected errors among the 51 unchecked findings. diagnostics.residual_sum is n/a for those 14 and is not comparable across the two groups.
- CODE FIX: scripts/edge_score.py crashed its own summary print with TypeError on None when a name has no adversary verdicts (residual_sum and edge_score_legacy both None). Since the adversary is now skipped four days in five that would have broken the summary on every normal run. Formatted both defensively; edge-scores.json was already written correctly before the crash, so no data was affected. smoke_test.py passes.
