...
## Capture — 2026-09-03 — GWRE tripwires investigated (3, all false positive, new class found)
- Logged at 2026-09-03 15:25 UTC
- Doc ed4ef72...653ae (thecerbatgem.com preview): 'post earnings of $0.93 per share' is the verb 'post' + noun 'earnings' in a forward-looking analyst-expectation sentence, not the adjective 'post-earnings'. Pre-print preview; stands.
- Doc 633c258...1bdee (stocktwits.com/symbol/GWRE/sentiment) and doc 5fedc08...4bbb (stocktwits Braze/Guidewire/UiPath roundup): both are aggregator pages. The 'beat'/'shares surged after reported' language is attributed to OTHER tickers on the same page (AEYE, Snowflake in doc1; the roundup covers GWRE's actual prior-year Sept-2025 Q4 print, not 2026-09-03, in doc2). Neither leaks the 2026-09-03 GWRE outcome.
- NEW FALSE-POSITIVE CLASS for scripts/capture.py's scan_tells: the event-date-form check ('2026-09-03' etc.) matched embedded JSON metadata elsewhere on the aggregator page (an unrelated poll's createdAt, a sidebar article's created_at) rather than the article's own date -- not previously documented in FINDINGS.md's false-positive catalogue (render-date, URL-slug). Worth a script fix: scan only the extracted article text's own date claims, not raw JSON blobs trafilatura leaves in. Flagging for maintainers, not fixed mid-routine.
- All 3 GWRE docs stand as legitimate pre-print content.

## Capture — 2026-09-03 — ZS tripwire investigated (false positive)
- Logged at 2026-09-03 15:26 UTC
- Doc ad8831f...60cc8b (fool.com preview, published 2026-09-02, titled 'Zscaler's Next Earnings Report on September 3 Could Send the Stock Soaring'): both tell matches are legitimate base-rate context, not outcome leakage -- 'Palo Alto's stock is falling despite topping estimates' refers to peer PANW's own recent print (area 9 read-through); 'stock fell sharply after the report... fiscal third-quarter report' refers to ZS's OWN PRIOR quarter (fiscal Q3), the guidance the upcoming Q4/Sept-3 print will be judged against (FINDINGS.md sec 5 pattern). Neither sentence describes the 2026-09-03 event's own outcome. Document stands.

## Capture — 2026-09-03 — CIEN tripwire investigated + same-day-BMO finding
- Logged at 2026-09-03 15:27 UTC
- 1 tripwire in 17 docs: bitcoinethereumnews.com's 'post-earnings move' is a hypothetical in a forward-looking technical-analysis sentence ('a failure to hold... on any post-earnings move would be a warning sign'), written before the outcome was known. False positive; stands.
- IMPORTANT: CIEN reported fiscal Q3 2026 results this morning (BMO, ~7:00am ET) BEFORE this agent-layer sweep ran (~15:15 UTC). The corpus for CIEN-2026-09-03 therefore includes CIEN's own earnings press release (stocktitan.net) and the EX-99.2 earnings-presentation exhibit from the just-filed 8-K -- genuine post-earnings content -- but the tells tripwire did NOT flag either document, because formal PR/8-K phrasing does not match the blunt beat/miss/shares-moved regex. This is a structural miss, not a bug to fix mid-routine: capture is deliberately session-agnostic (FINDINGS.md: no fence at capture time), and seal.py applies the real fence later from the EDGAR 8-K acceptance timestamp, so the actual pre-vs-post split will still be correct at replay. The finding for the routine is a coverage one: for a same-day BMO print, the daily agent-layer sweep (which runs mid-afternoon UTC / late morning ET) captures the immediate aftermath rather than genuine run-in positioning. Worth considering: running the agent-layer researchers before US market open for BMO-same-day names, or accepting that same-day BMO agent captures mainly document the announcement, not the setup.

## Capture — 2026-09-03 — DONE
- Logged at 2026-09-03 15:28 UTC
- Window swept: 2026-09-03 -> 2026-09-18 (15-day horizon).
- Script-only sweep (step 2): 208 events, every scheduled US name, no cap floor. 1254 new documents (filings pointers + quote bars + StockTwits deltas for near-dated names). 1 mechanical error: LEN.B (dual-class ticker, shares CIK -- known failure class per FINDINGS.md sec 3). 0 tripwires in the script sweep.
- Agent layer (step 3): the 6 largest names by market cap reporting within 3 days, all confirmed reporting 2026-09-03 except CPRT (see below) -- CIEN $56.6B, CPRT $30.3B, ZS $30.3B, IOT $24.4B, GWRE $16.7B, LULU $12.5B.
- Agent layer totals: 208 queries issued across the 6 names, 111 new documents stored (bodies -- 0 snippet-only, every planned URL that was fetched returned a usable body), 7 tripwires fired and investigated.
- Per name: CIEN q=40 docs=17 trip=1 | CPRT q=45 docs=15 trip=1 | ZS q=24 docs=12 trip=1 | IOT q=42 docs=23 trip=0 | GWRE q=32 docs=22 trip=3 | LULU q=25 docs=22 trip=1.
- Tripwire outcome: 6 of 7 confirmed false positive on close reading (prior-quarter recaps, peer read-through, a forward-looking 'post-earnings move' hypothetical, and a NEW false-positive class -- aggregator/StockTwits pages embedding today's date in unrelated sidebar-article or poll metadata rather than the article's own date). The 7th (CIEN) is a genuine same-day-BMO structural miss: CIEN reported before this sweep ran, so its own PR and 8-K exhibit are real post-earnings content that the blunt tells regex did not catch; documents stand because seal.py fences by the true EDGAR acceptance time at replay, not by capture-time heuristics. Full detail on each in the sections above.
