...
`researcher_us/scripts/edge_score.py` costs the stage about 0.15 of rank correlation, which is the
...
`researcher_us/scripts/edge_trade.py`. Entry is the close before the print, exit the close after the
...
impact sum is a coin flip. `researcher_us/scripts/edge_direction.py`; the per-event table is below and
`researcher_us/analysis/edge-direction.json` carries it machine-readable.
...
those are the names the sweep rated most promising. `researcher_us/scripts/edge_hunter_control.py` runs
the separations the data allows; `researcher_us/analysis/edge-hunter-control.json` is its output.
...
`researcher_us/scripts/edge_score.py` used to re-size every finding to the **mean** of the hunter's
...
news. `researcher_us/scripts/edge_exit.py` re-resolves all 38 de-duplicated events at eight exit
horizons against the same unchanged entry, off 5-minute Yahoo bars with
`includePrePost=true`. Output in `researcher_us/analysis/edge-exit.json`.
...
close while the money peaks before the open. `researcher_us/scripts/edge_exit.py` also prices every hour
...
name, mean **per trade**. Chart in `researcher_us/analysis/edge-exit-hourly.html`.
...
`edge_decompose.py` does it. Output in `researcher_us/analysis/edge-exit-forward.json`.
...
`researcher_us/scripts/edge_exit.py` prints both columns side by side and flags the degenerate one.
