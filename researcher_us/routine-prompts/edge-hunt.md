...
**2026-09-18 — `edge/` is now `researcher_us/`, and the frozen block below was left
alone on purpose.** The directory moved; `edge` is a symlink to it, so every path the
block names still resolves. The block still says `scripts/edge_score.py`,
`scripts/priced_in.py` and `.claude/skills/earnings-edge-hunt/SKILL.md`, and those three
are exactly what step 0 verifies before the run does anything, so none of them was
renamed. **The next paste is the moment to finish the rename**, and it is a five-line
diff plus one deletion:

| in the block, replace | with |
| --- | --- |
| `scripts/edge_score.py` | `researcher_us/scripts/edge_score.py` |
| `scripts/edge_universe.py` | `researcher_us/scripts/edge_universe.py` |
| `scripts/priced_in.py` | `researcher_us/scripts/priced_in.py` |
| `scripts/alpaca_trade.py` | `researcher_us/scripts/alpaca_trade.py` |
| `.claude/skills/earnings-edge-hunt/` | whatever the skill is called after it is renamed |

Only after that paste, and only once the Routine's `updated_at` confirms it, may the four
shims in `scripts/` and the `edge` symlink be deleted. Deleting either first stops the
next unattended run at step 0, before step 0b sells the previous day's book.
...
**The block below still names `scripts/…`, and the scripts moved to `researcher_us/scripts/`.**
That is deliberate and it is not stale: this file's job is to hold the exact text the
Routine holds, and the Routine cannot be edited from a session. Four forwarding shims
were left at `scripts/edge_score.py`, `scripts/edge_universe.py`, `scripts/priced_in.py`
and `scripts/alpaca_trade.py` so every command in the block, and the existence check in
step 0, still work unchanged. **On the next paste, prefix those five paths with `edge/`
and delete the shims** — that is the whole diff, and the shims' own docstrings say the
same. Until then the prose above and below the block uses the real paths and the block
uses the frozen ones; a reader who finds that jarring is reading it correctly.

**The ranking key reached `main` on `claude/agent-performance-comparison-wtwi1f`**, which
is fully merged. The prompt below never restated the scoring contract — it points at the
skill in the tree it actually cloned — so it survived that merge without an edit, which is
the property to keep on every future change.
...
after the day's book was placed by hand. Both need `researcher_us/scripts/alpaca_trade.py`, which
reached `main` on 2026-09-10 — that day's scheduled run found both steps no-ops because
the script, the `execution` block and `researcher_us/EXECUTION.md` were all still on
...
`researcher_us/LESSONS.md` before they search and return two numbers per name (the print against
...
the 09-16 run cloned a tree with no `researcher_us/LESSONS.md` in it and an `unpriced-hunter`
...
`researcher_us/LESSONS.md`, so the file's own effect on the ranking is measurable — the step-0b
...
0b. SELL YESTERDAY'S BOOK, BEFORE THE HUNT. Ask the code rather than reading the config by eye:
   python3 researcher_us/scripts/alpaca_trade.py mode
