...
BEST horizon beats the observed best.
    ...
    python3 researcher_us/scripts/edge_exit.py --cache /tmp/bars
    python3 researcher_us/scripts/edge_exit.py --out researcher_us/analysis/edge-exit.json
"""
import argparse
import glob
import json
...
def build_panel_from_runs(runs, cache):
    ...
    `researcher_us/analysis/edge-rows.json` yet -- yesterday's, or one still resolving -- can be scored."""
    panel, problems = [], []
    for run in runs:
        rp = Path(run)
...
def main():
    ...
    ap.add_argument("--out", default="researcher_us/analysis/edge-exit.json")
    ...
    ap.add_argument("--runs", nargs="*",
                    help="edge/ dirs to score directly instead of researcher_us/analysis/edge-rows.json")
