...
`researcher_us/scripts/edge_exit.py` prices the clock on the way OUT. This prices it on the
...
this source and `researcher_us/EDGE_ANALYSIS.md` already has one finding resting on that trap.
...
open and at the next close. They disagree in `researcher_us/EDGE_ANALYSIS.md` -- the gap to
...
entry conclusion that only holds at one of them is not a conclusion.
    ...
    python3 researcher_us/scripts/edge_entry_clock.py
    python3 researcher_us/scripts/edge_entry_clock.py --floor 0
"""
import argparse
import json
...
def main():
    ...
    ap.add_argument("--out", default="researcher_us/analysis/edge-entry-clock.json")
    a = ap.parse_args()
