...
the entry every headline number in `researcher_us/EDGE_ANALYSIS.md` was measured at. A plain
...
this question has an answer rather than an opinion:
  ...
  close  ->  next close   researcher_us/analysis/edge-rows.json, the `move` edge_resolve.py computes
  14:00  ->  next close   researcher_us/analysis/edge-direction.json, the scheme claude_naive priced

14:00 is an hour later than the hunt typically finishes, so it flatters the market
order slightly. Both are the same exit.

    python3 researcher_us/scripts/edge_entry_timing.py
"""
import json
import math
...
seen, cc = set(), {}
for day in sorted(json.loads(Path("researcher_us/analysis/edge-rows.json").read_text()),
...
mkt = {(r["t"], r["event"], r["sess"]): r
       for r in json.loads(Path("researcher_us/analysis/edge-direction.json").read_text())["rows"]}
