...
`researcher_us/EDGE_ANALYSIS.md` -- decomposed six resolved runs and found that every
...
should doubt a floor-clearing row, from researcher_us/LESSONS.md). None of it changes the key;
...
hunter had before it opened researcher_us/LESSONS.md, frozen by the hunter itself. It is the
...
realised move, so a file that only adds plausible rules can be caught doing it.
    ...
    python3 researcher_us/scripts/edge_score.py --run research/2026/09/2026-09-09/edge
    python3 researcher_us/scripts/edge_score.py --run <dir> --legacy   # old categorical files
"""
import argparse
import json
import math
...
def score_name(ticker, baseline, hunts, verdicts, legacy=False):
    ...
    # freezes that draft into `pre_lessons`, and only then reads researcher_us/LESSONS.md and
    ...
    if new_contract and hunts and not pre:
        flags.append("no pre-lessons draft: this name cannot be used to score "
                     "researcher_us/LESSONS.md, and the hunter that ran may not be the "
...
def main():
    ...
    doc = {
        ...
        "note": "Ranked on impact_sum: the hunters' signed per-finding sizes, added "
                ...
                "nothing -- see researcher_us/EDGE_ANALYSIS.md.",
        "ranking": rows,
    }
