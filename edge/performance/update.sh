...
#   ./edge/performance/update.sh --serve      rebuild, then serve it so the page's own
#                                             refresh button works (localhost only)
#   ./edge/performance/update.sh --serve-bg   the same, detached: you get your shell back
...
if curl -fsS --max-time 1 "http://127.0.0.1:$PORT/ping" >/dev/null 2>&1; then
  ...
  echo "      of: ./edge/performance/update.sh --serve-bg   (dan werkt de knop)"
fi
...
if (( SERVE == 1 )); then
  exec python3 edge/performance/scripts/serve.py --port "$PORT" \
...
if (( SERVE == 2 )); then
  ...
  if curl -fsS --max-time 1 "http://127.0.0.1:$PORT/ping" >/dev/null 2>&1; then
    ...
    nohup python3 edge/performance/scripts/serve.py --port "$PORT" \
