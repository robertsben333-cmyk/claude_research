...
that you are inside it, and the rule that follows. The hunter reads it before it starts
(`.claude/agents/unpriced-hunter.md` says so), and it is the only research guidance the
...
How it grows: `edge/scripts/edge_postmortem.py` scores each resolved run finding by
