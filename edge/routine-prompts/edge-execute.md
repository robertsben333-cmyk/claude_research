...
`edge/routine-prompts/edge-hunt.md`, which is the file to keep in step with
...
`edge/EDGE_ANALYSIS.md` was measured at.

## Three things still have to be true

1. **`edge/scripts/alpaca_trade.py` is on `main`.** Done on 2026-09-10: a Routine clones the
...
2. **The credentials are on the environment**, not in a shell: `ALPACA_API_KEY_ID`,
   ...
   `.env` format. `edge/EXECUTION.md` step 3 has the walkthrough.
...
pay (`edge/EXECUTION.md`, "The exit the two sessions actually want"). They differ only
