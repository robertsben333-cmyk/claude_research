...
**The block below still names `scripts/…`, and the scripts moved to `edge/scripts/`.**
...
after the day's book was placed by hand. Both need `edge/scripts/alpaca_trade.py`, which
reached `main` on 2026-09-10 — that day's scheduled run found both steps no-ops because
the script, the `execution` block and `edge/EXECUTION.md` were all still on
...
0b. SELL YESTERDAY'S BOOK, BEFORE THE HUNT. Ask the code rather than reading the config by eye:
   ...
   `close`, not `flatten`, while `orders.flatten_before_entry` is false - which it is, and it has to be while `exit_mode` is `auction_split` or the flatten sells every amc leg hours before its opening auction and cancels the working `cls` orders on the bmo legs. `close` sends what is due today with the instrument the session maps to, sends anything overdue at plain market, and waits `orders.fill_check_seconds` to check its own fills before it returns.
