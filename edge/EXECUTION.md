...
`edge/scripts/alpaca_trade.py` turns stage E's ranking into real orders and closes them
...
That is the whole selection. It is the only cut in `edge/EDGE_ANALYSIS.md` that
...
Regenerate that table with `python3 edge/scripts/edge_turnover_floor.py`, which also lists
...
6. **Re-paste the stage E Routine prompt** from `edge/routine-prompts/edge-hunt.md`,
...
`edge/scripts/edge_entry_timing.py` regenerates it. So the auction buys nothing worth a
...
python3 edge/scripts/alpaca_trade.py plan --run research/2026/09/2026-09-09/edge        # step 7
python3 edge/scripts/alpaca_trade.py open --run research/2026/09/2026-09-09/edge --submit --no-flatten
...
the start of the next run. `edge/scripts/edge_exit.py` re-priced all 38 de-duplicated events
...
- **A second run a day, for `auction_split` only.** Alpaca *rejects* rather than queues
  ...
  bounce. The prompt is in `edge/routine-prompts/edge-execute.md` and **only a person can
