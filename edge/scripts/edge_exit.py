...
BEST horizon beats the observed best.
    ...
    python3 edge/scripts/edge_exit.py --cache /tmp/bars
    python3 edge/scripts/edge_exit.py --out edge/analysis/edge-exit.json
"""
import argparse
import glob
import json
...
def build_panel_from_runs(runs, cache):
    ...
    `edge/analysis/edge-rows.json` yet -- yesterday's, or one still resolving -- can be scored."""
    panel, problems = [], []
    for run in runs:
        rp = Path(run)
...
def main():
    ...
    ap.add_argument("--out", default="edge/analysis/edge-exit.json")
    ...
    ap.add_argument("--runs", nargs="*",
                    help="edge/ dirs to score directly instead of edge/analysis/edge-rows.json")
