...
ONE ENGLISH PASS PER HUNTER, AND THAT IS DELIBERATE. There is no Australian-language press the wires do not read, so there is no second language to search and nothing to note. unpriced-hunter-au has no pre_local field and no language_note field. Do not ask a hunter for either and do not add them to the contract - a field that would be structurally empty on every Australian name is worse than absent, because somebody would eventually pool it. Since 2026-09-22 stages EU and CA run ONE bilingual pass each and have dropped their own pre_local freeze, so this is no longer the exception it was; the difference that remains is that they have a second language to search and Australia does not. The pre_lessons freeze still runs, and researcher_australia/LESSONS.md is deliberately empty until a run resolves, so "nothing changed" is the correct answer there and not a fault.
