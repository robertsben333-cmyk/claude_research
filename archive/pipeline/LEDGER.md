...
Nine runs scored so far (2026-08-19, 2026-08-26, 2026-08-27, 2026-09-01, 2026-09-02,
2026-09-04, 2026-09-07, 2026-09-08, 2026-09-16). 2026-08-31 and 2026-09-17 were checked
and found to have nothing to score (stages 0-2/no eligible names that day — see their
own `05-outcome.md`). The 2026-09-08 run (TTAN, CHWY, plus four non-panelled names)
resolved on the 2026-09-09 close and was scored 2026-09-10 — but that scoring only
reached `research/2026/09/2026-09-08/`; it was produced on a branch that got merged back
into `main` on 2026-09-10 for its *research* files, and this ledger's own summary/table
were never updated to include it until a later run closed that gap. 2026-09-09 through
2026-09-15 were checked and found **not scorable**: none has an `04-advice.json` with
any names to score (either no advice file at all, or an empty/no-names shortlist) — so
there is nothing yet to score for those days. **n=16 for panelled calls.**

**LEN (2026-09-16) is the panel's first directional call — Lean Down, certainty
High — and its first miss on direction.** Every one of the prior 15 panelled calls was
Neutral / No Edge; this is also the first panelled call with a real signed estimate, so
it produces the ledger's first-ever non-null magnitude error. See the Runs table below
for the full writeup — the fundamentals missed exactly as called, but a same-day Fed
rate decision overran the reaction, a measurement-window limitation rather than a
misread of the company.

| Metric | Panelled calls | Deep-dive preliminary reads |
| --- | --- | --- |
| Calls scored | 16 | 36 |
| Direction hit rate | 56% (9/16, Neutral-hit convention for the 15 Neutral calls, sign-match for LEN) | 42% (15/36) |
| Mean absolute magnitude error | 3.11 (n=1 — LEN is the only panelled call ever to carry a signed estimate) | n/a (no magnitude estimate) |
| Median absolute magnitude error | 3.11 (n=1) | n/a |
| Band hit rate | 56% (9/16) | n/a |
| Implied move broken | 38% (6/16) | n/a |
| Red-team reversal fired | 38% (6/16) | n/a |

**Panel vs. single deep researcher, sixteen panelled calls in — but read the n
carefully:** fourteen of sixteen panelled calls — WOLF, OKTA, DLTR, AFRM, ESTC, DELL,
CXM, AI, SNOW, ABM and UNFI (each scored *twice*, from two independent re-panels three
days apart around the same 2026-09-08 print), and TTAN and CHWY — have been **Neutral /
No Edge**; only 2026-09-16's LEN broke that streak with a real directional call (Lean
Down), and it missed. The panelled hit rate slipped again, from 60% (9/15) to 56%
(9/16), on LEN's miss. **Treat the 56% figure as resting on 14 independent events, not
16** — the ABM/UNFI double count still applies — so the true underlying picture moves
from 7/13 (54%) to 7/14 (50%): a coin flip. The preliminary read went 0/1 on LEN (−22,
wrong-signed, the same call the panel got wrong and for the same underlying reason — a
same-day Fed decision neither method was tasked with pricing in), nudging its running
rate down from 43% (15/35) to 42% (15/36) — still below the panel's raw 56%, and the
...
| --- | --- | --- |
| High | 8 (OKTA miss, DLTR hit, ESTC miss, AI hit, SNOW miss, TTAN miss, CHWY hit, LEN miss) | 38% (3/8) |
| Med | 8 (WOLF hit, AFRM hit, DELL miss, CXM miss, ABM 09-04 hit, UNFI 09-04 hit, ABM 09-07 hit, UNFI 09-07 hit) | 75% (6/8) |
| Low | 0 | — |

LEN (2026-09-16) is the first *directional* call in this table — every entry before it
was scored under the below-expected-move convention. It landed in the High tier and
missed, which is the worst possible placement for the tiering's credibility: the one
call confident enough to pick a direction, at the highest certainty tier, on the largest
n this table has had, got the direction wrong. The gap now holds at six consecutive
updates: High-certainty calls hit 38% (3/8, down from 43%) while Med-certainty calls hit
75% (6/8, unchanged) — a 37-point gap on the metric this table exists to check. Caveat
as before: two of the eight Med calls are the same two events counted twice (ABM/UNFI,
both runs), so independent-event n for Med is really 6, not 8, and the true hit rate on
independent events is 4/6 (67%) — still comfortably above High's 38%. "High-certainty
calls hit no better, and usually worse, than Med" has now held across six consecutive
ledger updates. `scripts/synthesize.py`'s certainty logic needs a direct look at
whether disparity/alignment is the wrong signal to build the tier on — this is no
longer a call for patience on a bigger sample; the sample has grown for six updates
running and the direction of the gap has not moved once.
...
| 2026-09-09 to 2026-09-15 | Not scorable | — | — | — | Checked: none of these dates has an `04-advice.json` carrying any names to score (either no advice file at all, or an empty/no-names shortlist). Nothing published for stage 4 to check against reality. |
| 2026-09-16 | 1 (LEN) | 1 (LEN) | Panelled 0/1 (0%, direction sign-match — the panel's first non-Neutral call); Prelim 0/1 (0%) | +3.11 (n=1 — the ledger's first panelled magnitude error) | LEN: adj EPS $1.23 vs $1.30 consensus, revenue $8.05B vs $8.31B consensus, new orders −9% y/y, full-year delivery guidance cut a second time — a clean, sourced miss, exactly what the panel (consensus −25.7, Lean Down, certainty High, signed estimate −1.4%) and the prelim (−22) both called. Closed $78.36→$79.70, **+1.71%**, below even the low end of the panel's own 2.3–7.3% unsigned band and short of the 5.2% implied move — direction miss, band miss, implied-move-not-broken. The reaction was in line with the call intraday (after-hours down to ~$76.34, a 09-17 intraday low of $76.07, both below the pre-print close) before reversing to close positive on a **same-day Fed rate decision** that lifted the broader market — an exogenous, company-unrelated catalyst landing in the same close-to-close window, not a misread of Lennar. Red-team's named bull mechanism (mortgage-rate tailwind, a fresh buyback in the release) did not fire — there was no beat and no buyback announced — so `reversal_fired: false` even though the price technically moved against the call. First panelled call to break the Neutral/No-Edge streak (14 straight before it), and it missed at the panel's highest certainty tier — see the certainty-tier section above. |
