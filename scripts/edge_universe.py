#!/usr/bin/env python3
"""Compatibility shim. The real script is `researcher_us/scripts/edge_universe.py`.
...
Delete this file only after `researcher_us/routine-prompts/edge-hunt.md` has been
...
`researcher_us/scripts/`.
"""
import runpy
import sys
from pathlib import Path

TARGET = Path(__file__).resolve().parent.parent / "researcher_us" / "scripts" / "edge_universe.py"

if __name__ == "__main__":
    # Guarded, so that importing this name by accident does not run a CLI. Anything
    # that wants the module itself should put `researcher_us/scripts` on sys.path and import
