...
`scripts/edge_direction.py` (entry 14:00 ET, exit next open / next close) and the
ranking correlation of `scripts/edge_decompose.py` (impact sum vs realised move,
pooled within days).

    python3 scripts/edge_hunter_control.py
"""
import argparse
import glob
import json
...
def main():
    ...
    ap.add_argument("--rows", default=str(REPO / "docs/edge-rows.json"))
    ap.add_argument("--cache", default=str(REPO / ".edge_direction_cache.json"))
    ap.add_argument("--out", default=str(REPO / "docs/edge-hunter-control.json"))
